CKEDITOR.config.toolbar_MTFull =
[
	['Source','-','Save','NewPage','Preview','-','Templates'],
	['Cut','Copy','Paste','PasteText','PasteFromWord','-','Print', 'SpellChecker', 'Scayt'],
	['Undo','Redo','-','Find','Replace','-','SelectAll','RemoveFormat'],
	['Form', 'Checkbox', 'Radio', 'TextField', 'Textarea', 'Select', 'Button', 'ImageButton', 'HiddenField'],
	'/',
	['Bold','Italic','Underline','Strike','-','Subscript','Superscript'],
	['NumberedList','BulletedList','-','Outdent','Indent','Blockquote'],
	['JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock'],
	['Link','Unlink','Anchor'],
	['Image','Flash','Table','HorizontalRule','Smiley','MobilePictogram','SpecialChar','PageBreak'],
	['mt_insert_image', 'mt_insert_file'],
	'/',
	['Styles','Format','Font','FontSize'],
	['TextColor','BGColor'],
	['Maximize', 'ShowBlocks','-','About']
];

CKEDITOR.config.toolbar_MTNormal =
[
	// Issue # 13871
	['Source','Bold','Italic','Underline','Strike','Subscript','Superscript'],
	['NumberedList','BulletedList','Blockquote'],
	['JustifyLeft','JustifyCenter','JustifyRight'],
	['Image','Flash','Table','HorizontalRule','PageBreak'],
	['mt_insert_image', 'mt_insert_file'],
	'/',
	['Format','Font','FontSize'],
	['TextColor','BGColor','MobilePictogram','SpecialChar'],
	['Link','Unlink','Anchor'],
	['Maximize', 'ShowBlocks']
];

CKEDITOR.config.toolbar_MTSimple =
[
	['Bold','Italic','Underline','Strike','-','Subscript','Superscript'],
	['NumberedList','BulletedList',],
	['JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock'],
	['Link','Unlink','Anchor'],
	['Image','Flash','HorizontalRule','SpecialChar'],
	['mt_insert_image', 'mt_insert_file'],
	'/',
	['Styles','Format','Font','FontSize'],
	['TextColor','BGColor'],
	['Paste','PasteText','PasteFromWord'],
	['SelectAll','RemoveFormat'],
];

CKEDITOR.config.toolbar_MTTable =
[
	['Bold','Italic','Underline','Strike','-','Subscript','Superscript'],
	['NumberedList','BulletedList',],
	['JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock'],
	['Link','Unlink','Anchor'],
	['Image','Flash','Table','HorizontalRule','SpecialChar'],
	['mt_insert_image', 'mt_insert_file'],
	'/',
	['Styles','Format','Font','FontSize'],
	['TextColor','BGColor'],
	['Paste','PasteText','PasteFromWord'],
	['SelectAll','RemoveFormat'],
];
