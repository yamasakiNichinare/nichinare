...
(function(aform$) {
	...
	aform$.extend(aform$.ui, {
		plugin: {
			add: function(w, c, o, p) {
				var a = aform$.ui[w].prototype; if(!a.plugins[c]) a.plugins[c] = [];
	...
	aform$.fn.mouseInteractionDestroy = function() {
		this.each(function() {
			if(aform$.data(this, "ui-mouse")) aform$.data(this, "ui-mouse").destroy(); 	
		});
	}
	
	aform$.ui.mouseInteraction = function(el,o) {
	
		if(!o) var o = {};
		this.element = el;
		aform$.data(this.element, "ui-mouse", this);
		
		this.options = {};
		aform$.extend(this.options, o);
		aform$.extend(this.options, {
			handle : o.handle ? (aform$(o.handle, el)[0] ? aform$(o.handle, el) : aform$(el)) : aform$(el),
		...
		if(!this.options.nonDestructive && (o.helper == 'clone' || o.helper == 'original')) {
			...
			o.margins = {
				top: parseInt(aform$(el).css('marginTop')) || 0,
				left: parseInt(aform$(el).css('marginLeft')) || 0,
				bottom: parseInt(aform$(el).css('marginBottom')) || 0,
				right: parseInt(aform$(el).css('marginRight')) || 0
			};
			...
			if(o.helper == 'original')
				o.wasPositioned = aform$(el).css('position');
		...
		if(aform$.browser.msie) aform$(this.element).attr('unselectable', 'on');
		
	}
	
	aform$.extend(aform$.ui.mouseInteraction.prototype, {
		plugins: {},
		currentTarget: null,
		lastTarget: null,
		timer: null,
		...
		click: function(e) {
			...
			var initFunc = function() { //This function get's called at bottom or after timeout
				aform$(document).bind('mouseup', self.mouseup);
				aform$(document).bind('mousemove', self.mousemove);
		...
		start: function(e) {
			...
			o.co = aform$(a).offset(); //get the current offset
				
			this.helper = typeof o.helper == 'function' ? aform$(o.helper.apply(a, [e,this]))[0] : (o.helper == 'clone' ? aform$(a).clone()[0] : a);

			if(o.appendTo == 'parent') { // Let's see if we have a positioned parent
				var cp = a.parentNode;
				while (cp) {
					if(cp.style && (aform$(cp).css('position') == 'relative' || aform$(cp).css('position') == 'absolute')) {
						o.pp = cp;
						o.po = aform$(cp).offset();
						o.ppOverflow = !!(aform$(o.pp).css('overflow') == 'auto' || aform$(o.pp).css('overflow') == 'scroll'); //TODO!
						break;	
					}
			...
			if(!o.nonDestructive) aform$(this.helper).css('position', 'absolute');
			if(o.helper != 'original') aform$(this.helper).appendTo((o.appendTo == 'parent' ? a.parentNode : o.appendTo)).show();
		...
		stop: function(e) {			
			...
			aform$(document).unbind('mouseup', self.mouseup);
			aform$(document).unbind('mousemove', self.mousemove);
			...
			if(this.helper != a && !o.beQuietAtEnd) { // Remove helper, if it's not the original node
				aform$(this.helper).remove(); this.helper = null;
			}
			
			if(!o.beQuietAtEnd) {
				//if(o.wasPositioned)	aform$(a).css('position', o.wasPositioned);
		...
		drag: function(e) {
			...
			if (!this.opos || (aform$.browser.msie && !e.button)) return this.stop.apply(this, [e]); // check for IE mouseup when moving into the document again
			var o = this.options;
