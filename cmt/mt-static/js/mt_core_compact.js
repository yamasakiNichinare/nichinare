...
m=this.getMonth();return this.constructor.strings.localeShortMonths[m];}});if(!defined(window.Enumerator)){window.Enumerator=new Class(Object,{init:function(a){this.data=a;this.index=0;},atEnd:function(){return this.index>=this.data.length?true:false;},item:function(){return this.atEnd()?undefined:this.data[this.index];},moveFirst:function(){this.index=0;return this.item();},moveNext:function(){this.index++;return this.item();}});}
if(!defined(window.XMLHttpRequest)){window.XMLHttpRequest=function(){var h=["Microsoft.XMLHTTP","MSXML2.XMLHTTP.5.0","MSXML2.XMLHTTP.4.0","MSXML2.XMLHTTP.3.0","MSXML2.XMLHTTP"];for(var i=0;i<h.length;i++){try{return new ActiveXObject(h[i]);}catch(e){}}
...
w=window;return w.getSelection?w.getSelection():(d?d.selection:w.document.selection)},mergeTextNodes:function(n){var c=0;while(n){if(n.nodeType==Node.TEXT_NODE&&n.nextSibling&&n.nextSibling.nodeType==Node.TEXT_NODE){n.nodeValue+=n.nextSibling.nodeValue;n.parentNode.removeChild(n.nextSibling);c++;}else{if(n.firstChild)
...
return undefined;var parentDocument=pw.document;var es=parentDocument.getElementsByTagName("iframe");for(var en=new Enumerator(es);!en.atEnd();en.moveNext()){var e=en.item();try{var d=e.contentDocument||e.contentWindow.document;if(d===nd)
return e;}catch(err){};}
return undefined;},filterElementsByClassName:function(es,cn){var filtered=[];for(var en=new Enumerator(es);!en.atEnd();en.moveNext()){var e=en.item();if(DOM.hasClassName(e,cn))
filtered.push(e);}
return filtered;},filterElementsByAttribute:function(es,a){if(!es)
return[];if(!a)
return es;var f=[];for(var en=new Enumerator(es);!en.atEnd();en.moveNext()){var e=en.item();if(!e)
continue;try{if(e.getAttribute&&e.getAttribute(a))
f.push(e);}catch(e){}}
return f;},filterElementsByTagName:function(es,tn){if(tn=="*")
return es;var f=[];tn=tn.toLowerCase();for(var en=new Enumerator(es);!en.atEnd();en.moveNext()){var e=en.item();if(e.tagName&&e.tagName.toLowerCase()==tn)
f.push(e);}
...
return;var es=DOM.getAncestors(ev.target,true);for(var en=new Enumerator(es);!en.atEnd();en.moveNext()){var e=en.item();try{var v=e.getAttribute?e.getAttribute(a):null;if(v){ev.attributeElement=e;ev.attribute=v;return v;}}catch(e){}}},setElementAttribute:function(e,a,v){if(navigator.userAgent.toLowerCase().match(/webkit/)){var at=e.attributes;for(var i=0;i<at.length;i++)
if(at[i].name==a)
