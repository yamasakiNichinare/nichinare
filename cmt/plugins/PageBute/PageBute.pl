...
my %garbage = (
     ...
     IFPAGENEXT_END    => '<!-- PageIfNext end for PageBute -->',
...
sub _if_pagination_ {
   ...
   my $key = {
      ...
      ifpaginationdynamic => '__is_dynaimc__',
...
sub _page_bute {
    ...
    $pb->{PAGENEXT_delimitor}   ||= $delimitor{PAGENEXT};
	...
	for (my $i=0; $i < $#entries; $i++) {
		if( ($i + 1) % $split_count == 0 || $i == $#entries - 1) {
			...
			#replace next link
			if ($lists->{next}) {
				$next = _create_page_link( $ctx,$page_link_format, $page_count + 1, $base_url, $suffix, $pb->{PAGENEXT_delim}, 'link_next' );
				$next = $pb->{link_start}. $next. $pb->{link_close};
				$output =~ s/\Q$garbage{IFPAGENEXT}\E|\Q$garbage{IFPAGENEXT_END}\E//g;
...
sub _page_bute_cgi {
    ...
    if ($pinfo->{next}) 
    {
        $out = _create_page_link_cgi( $ctx , $page_link_format, $page + 1, $url , $pb->{PAGENEXT_delim}, 'link_next' );
        $out = $pb->{link_start}. $out. $pb->{link_close};
        $contents =~ s/\Q$garbage{IFPAGENEXT}\E|\Q$garbage{IFPAGENEXT_END}\E//g;
        $contents =~ s/\Q$garbage{PAGENEXT}\E/$out/g;
    }
    else
    {
        $contents =~ s/\Q$garbage{IFPAGENEXT}\E[\s\S]*?\Q$garbage{IFPAGENEXT_END}\E//g;
    }
