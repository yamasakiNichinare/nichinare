...
sub _param_exports {
    ...
    while ( my $website = $iter->() ) {
         ...
         push @exports , {
            ...
            indent   => 0,
            selected => 0,
         } if $children && $class ne 'entry' && $child_allow && $this_allow;
    ...
    unshift @exports , {
          ...
          indent   => 0,
          selected => 0,
    } if $class ne 'entry' && $allow_website;
...
sub csv_export {
    ...
    foreach my $blog (@blogs) {
        my @CustomFields = CustomFields::Field->load({ blog_id => $blog->id, obj_type => $param_class });
    ...
    #
    my @CustomFields = CustomFields::Field->load({ blog_id => 0, obj_type => $param_class });
    foreach my $CustomField (@CustomFields) {
        my $basename = $CustomField->basename;
        push @buf_columns, $basename;
    }
    ...
    if ( $app->version_number =~ /^5/){
        $encode_switch = sub { return _encode_text($_[2], $_[3], MT::I18N::utf8_off($_[0]), $_[1], $encoding ); };
    }else{
        $encode_switch = sub { return _encode_text($_[2], $_[3], $_[0], $_[1], $encoding ); };
    }
    ...
    foreach my $blog (@blogs) {
        my @entries = MT::Entry->load({ blog_id => $blog->id, class => $param_class });
...
sub csv_import {
    ...
    # do importing
    my $ret = _import_csv_file ($app, $fh, \%param, $update_flg, $class);
    if (defined $ret) {
        my $tmpl = $plugin->load_tmpl( $ret );
        return $app->build_page( $tmpl, \%param );
    }
...
sub _import_csv_file {
    my ($app, $fh , $param_ref, $update_flg, $class, $filename, $encoding) = @_;
    ...
    foreach my $data (@data_buf) {
        ...
        eval {
            $ret = &_save_entry( $plugin, $data, $update_flg, $class , $line_count);
        };
        if ($@) {
            & _print_mt_log (
                    $plugin->translate ('[_1] Line', $line_count). ' '.
    ...
    if ($error_count > 0) {
        ...
        $msg .= $plugin->translate( '<a href="mt.cgi?__mode=view_log">detail log</a>' );
    }
...
sub _allow_blogs {
    ...
    unless ( $flag ) {
       ...
       return wantarray ? grep { $allow_blogs{$_} == 1 } keys %allow_blogs : \%allow_blogs;
...
### import data save
sub _save_entry {
    my ($plugin, $data, $update_flg, $class, $line_count) = @_;
    my $app = MT->instance;
    my $entry;
    my $entry_id = $data->{entry_id};
    my $blog_id  = $data->{entry_blog_id};
    ...
    # data check
    my $err_msg = &_data_check( $plugin, $data, $entry_id, $blog_id, $class );
    if ($err_msg) {
...
#            $primary_cate = ''; # clear it
 ...
 #           $primary_cate = ''; # clear it
        ...
        foreach my $column (@entry_columns) {
             if ($key eq 'entry_' . $column){
                 if ($column eq 'category_id') {
                     ...
                     if ($value) {
                         my $authored_time = HTTP::Date::str2time( $value );
                         if ($authored_time){
                             my @authored_ts = MT::Util::offset_time_list( $authored_time, $blog );
                             my $authored_ts = sprintf '%04d%02d%02d%02d%02d%02d', $authored_ts[5]+1900, $authored_ts[4]+1, @authored_ts[3,2,1,0];
                             $value = $authored_ts;
                         }
                         else {
                             $value = $ts;
    ...
    foreach my $CustomField (@CustomFields) {
        ...
        if ( $CustomField->type eq 'datetime' ) {
            if ( $data->{$basename} ) {
                 my $cf_datetime = HTTP::Date::str2time( $data->{$basename} ); 
                 if ($cf_datetime) {
                      my @cf_ts = MT::Util::offset_time_list( $cf_datetime, $blog );
                      $meta->{$basename} = sprintf '%04d%02d%02d%02d%02d%02d', $cf_ts[5]+1900, $cf_ts[4]+1, @cf_ts[3,2,1,0];
                 } else {
                      $meta->{$basename} = undef;
                 }
            } else {
...
### データチェック
sub _data_check {
    my ($plugin, $data, $entry_id, $blog_id, $class) = @_;
    my @values;
