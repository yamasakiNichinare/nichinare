...
function smarty_block_mtsortedsubcategories ($args, $content, &$ctx, &$repeat) {
    ...
    $result = _mtsubcategories ($args, $content, $ctx, $repeat);
    if (!$repeat) {
...
function cmp_ascend ($a, $b) { return ($a->order_number < $b->order_number) ? -1 : 1; }
function cmp_descend ($a, $b) { return ($a->order_number < $b->order_number) ? 1 : -1; }

function _mtsubcategories ($args, $content, &$ctx, &$repeat) {
...
if ($sort_order === 'ascend')
    usort($cats, "cmp_ascend");
else
    usort($cats, "cmp_descend");
