package MT::Plugin::SKR::MightyCMS::Abs2Rel;
#   Abs2Rel - Omit the domain name and base path from its URL, and make it only relative path.
...
my $plugin = __PACKAGE__->new({
        ...
        doc_link => 'http://www.skyarc.co.jp/engineerblog/entry/urlabs2rel.html',
        l10n_class  => $MYNAME. '::L10N',
        description => <<HTMLHEREDOC,
<MT_TRANS phrase="Omit the domain name and base path from its URL, and make it only relative path.">
HTMLHEREDOC
...
sub _hdlr_build_page {
    ...
    my $site_url = $blog->{column_values}->{archive_url} || $blog->site_url
        or return 1;
    my ($url_domain, $url_path) = $site_url =~ m!^(https?://[^/]+)(/[^#\?]*)?!;
    ...
    $$content =~ s!(<[^>]+(?:href|src|action)\s*=\s*(["']))([^'"]+)\2!$1.abs2rel($3, $url_domain, $base_pathname).$2!segi;

    ### PageBute Support
    my $ctx = $opt{Context};
    my $pb = $ctx->stash('PageBute') or return 1;
    $pb->{abs2rel} = 1;
    $pb->{contents} =~ s!(<[^>]+(?:href|src|action)\s*=\s*(["']))([^'"]+)\2!$1.abs2rel($3, $url_domain, $base_pathname).$2!segi;
