...
use vars qw( $VERSION $OLD_VERSION $URL %FORMATS %OPTIONS $I_EZ_MAX_WIDTH $IS_MT5 $XML_DECLARATION %DOCTYPE $QVGA_WIDTH $EMOJI_SIZES);
$VERSION = '1.61';
$OLD_VERSION = '1.30';
...
sub _hdr_config_template {
	...
	if($scope eq 'system') {
		...
		my $smartphone_remove_style = lc($param->{smartphone_remove_style});
...
sub _hdr_KeitaiKitSysTmpl {
	...
	if(is_dynamic || $args->{dynamic}) {
		...
		if(/^DoCoMo\/1\.0\/([^\/]+)/i || /^DoCoMo\/2\.0 ([^\(]+)/i) {
			$carrier = 'i';
			my $id = $1;
			$spec = $kdb->findSpec("$carrier$id");


			if(($spec->{img} & $FORMATS{MG_RESOLUTIONS}) != 0 && /TB[;\/](W[0-9]+H[0-9]+)/) {
			    my $key = $1;
			    my $alt_spec = $kdb->findSpec("$carrier${id}_$key");
...
sub _hdr_KeitaiInputStyleSysTmpl {
	...
	if($carrier eq 's') {
		...
		$html .= qq(istyles="$value");
	}
	

	if(($carrier eq 'i' && $ctx->stash('mtkk_ixhtml')) || $carrier eq 'ez') {
		my %modes = (1 => 'hiragana', 'katakana' => 'hankakukana', 2 => 'hankakukana', 3 => 'alphabet', 4 => 'numeric');
