...
if(strtolower($mtkk_php_encoding) != 'shift_jis' && strtolower($mtkk_php_encoding) != 'sjis') {
...
if(preg_match('/^DoCoMo\/1\.0\/([^\/]+)/i', $mtkk_ua, $matches)
	...
	$mtkk_carrier = 'ez';
	if(preg_match('/UP\.Browser\/([0-9]+)/i', $mtkk_ua, $submatches) && $submatches[1] >= 6)
		$mtkk_is_new_device = true;
		...
		|| preg_match('/^MOT\-([^\/]+)\/80.2F.2E.MIB/i', $mtkk_ua, $matchs)) {
	$mtkk_carrier = 's';
...
if($mtkk_carrier == 'i') {
    ...
    $no_vga = ($mtkk_spec['img'] & MG_NOVGA)? true: false;
    if($mtkk_ivga && $mtkk_spec['format1'] >= 2000 && !$no_vga) {
        $mtkk_spec['bw'] *= 2;
        $mtkk_spec['bh'] *= 2;
...
if($mtkk_carrier == 's') {
    ...
    if(isset($_SERVER['HTTP_X_S_DISPLAY_INFO'])) {
        $display_info = $_SERVER['HTTP_X_S_DISPLAY_INFO'];
        if(preg_match('/^([0-9]+)\*([0-9]+)/', $display_info, $matches)) {
	...
	if(preg_match('/iPhone|iPod|Android/i', $mtkk_ua, $matches)) {
		...
		if($device == 'iphone' || $device == 'ipod') {
			...
			if(preg_match('/iPhone OS ([0-9][0-9_]*)/', $mtkk_ua, $matches)) {
				$digits = explode('_', $matches[1]);
				$version = array_shift($digits);
				if(count($digits)) $version .= '.' . join('', $digits);
				$mtkk_smartphone['os_version'] = (float)$version;
			} else {
...
header("Content-Type: $mtkk_content_type; charset=Shift_JIS");
...
function mtkk_convert_to_sjis($buffer) {
  ...
  $buffer = mb_convert_encoding($buffer, 'SJIS-WIN', $mtkk_php_encoding);
...
function mtkk_output_xhtml_headers() {
	...
	if($xhtml_version) {
		...
		if($mtkk_xml_declaration)
			$xhtml_headers .= sprintf($mtkk_xml_declaration_format, 'Shift_JIS') . "\n";
...
function mtkk_build_url($cache_size, $url, $sw, $sh, $c, $img, $w, $h, $fit = null, $thumbnail = 0, $copyright = null, $wallpaper = null, $maximize = false, $cache_expires = null, $magnify = null) {
	...
	if(isset($w)) $url .= "&mtkk_w=$w";
	if(isset($h)) $url .= "&mtkk_h=$h";
	...
	if(isset($wallpaper)) $url .= "&mtkk_wallpaper=" . serialize($wallpaper);
	...
	if($size) {
		if($mtkk_multi_servers) {
			...
			$src = mtkk_build_url($kg->cache_size, $image['url'],
				$dw, $dh, $mtkk_spec['colors'], $mtkk_spec['img'],
				isset($image['w'])? $image['w']: null, isset($image['h'])? $image['h']: null, isset($image['fit'])? $image['fit']: null, 0, $copyright_arg, null, $maximize, 
...
function mtkk_paginate($buffer) {
	...
	$count_text_init = $count_init = strlen($buffer_top) + (int)$mtkk_footer_bytes;
	...
	for($i = 0; $i < $buffer_count; $i++) {
		...
		if($section_size == 0) {
			$section_text_size = $section_size = strlen($b);
...
function mtkk_input_style($style, $extra_style = '') {
	...
	if(($mtkk_carrier == 'i' && isset($mtkk_ixhtml) && $mtkk_ixhtml) || $mtkk_carrier == 'ez') {
		$modes = array(1 => 'hiragana', 'tatakana' => 'hankakukana', 2 => 'hankakukana', 3 => 'alphabet', 4 => 'numeric');
		$style = isset($modes[$style])? $modes[$style]: $style;
	    $styles = array(
            'hiragana' =>    '-wap-input-format:*M;-wap-input-format:&quot;*&lt;ja:h&gt;&quot;;',
            'hankakukana' => '-wap-input-format:*M;-wap-input-format:&quot;*&lt;ja:hk&gt;&quot;;',
            'alphabet' =>    '-wap-input-format:*m;-wap-input-format:&quot;*&lt;ja:en&gt;&quot;;',
            'numeric' =>     '-wap-input-format:*N;-wap-input-format:&quot;*&lt;ja:n&gt;&quot;;'
	    );
	    echo ' style="-wap-input-format:&quot;*&lt;' . (isset($styles[$style])? $styles[$style]: $style) . '&gt;&quot; ' . $extra_style . '"';
	}
}
