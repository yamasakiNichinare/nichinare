...
%Lexicon = (
    ...
    'Export all entries of blog(s) to CSV file format. And import entries from exported CSV data.' =>'ブログ記事・ウェブページをCSVフォーマットで一括登録・出力するプラグインです。',
    ...
    'If there is entry have a same entry_id, existing entry\'s data are over written.' => '同じエントリーIDが存在する場合、そのエントリーを上書きする。',
    ...
    '<a href="mt.cgi?__mode=view_log">detail log</a>' => ' 詳しくは<a href="mt.cgi?__mode=view_log">ログを確認</a>してください',
