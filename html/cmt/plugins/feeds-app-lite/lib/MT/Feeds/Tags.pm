...
use Encode;
use MT::Feeds::Util qw(decode_data );
...
sub feed_title {
    ...
    require MT::Util;
    my $title = decode_data($lite->find_title($lite->feed));
...
sub feed_link {
    ...
    my $lite = $ctx->stash(LITE)
      or return _error($ctx);
    my $link = decode_data($lite->find_link($lite->feed));
    sanitize_link($link);
}
...
sub entry_title {
    ...
    my $entry = $ctx->stash(ENTRY)
      or return _error($ctx);
    my $title = decode_data($lite->find_title($entry));
    $title = '' unless defined $title;
    $title = MT::Util::remove_html($title)
        unless (exists $args->{remove_html}) && !$args->{remove_html};
    $title = MT::Util::encode_html($title)
...
sub entry_link {
    ...
    my $entry = $ctx->stash(ENTRY)
      or return _error($ctx);
    my $link = decode_data($lite->find_link($entry));
    sanitize_link($link);
}
